...
melodia.src = "./sonidos/sonic-2.mp3";
...
function nave(x) {
    ...
    this.dibuja = function (x) {
        this.x = x;
        if(imgAni2 < 5) {
            ctx.drawImage(imgNave, this.x, this.y);
            imgAni2 = imgAni2 + 1;
            imgAni = imgAni + 1;
            checarBalas();
...
function Enemigo(x, y) {
    ...
    this.dibuja = function () {
        ...
        if(this.vive) {
            if (imgAni < 4) {
                ctx.drawImage(imgOvni, this.x, this.y);      
            }
            else if (imgAni < 8) {
                ctx.drawImage(imgOvni, this.x, this.y);
            }
...
function gameOver() {
    ...
    if (enemigosVivos == 0) {
        ...
        ctx.fillText("GaNaStE!", 28, 68);
        ctx.font = "bold 48px Arial";
        ctx.fillText("OtRa?? F5", 118, 128);
        ctx.fillStyle = "white";
        ctx.font = "bold 28px Courier";
        ctx.fillText("PUNTOS: " + puntos, 24, 440);
        ctx.font = "bold 22px Courier";
        ctx.fillText("MUNICIÓN: " + "  " + municion, 24, 465);
...
function verifica() {
    ...
    if (tecla[teclaEspacio]) {
        if (tiempoBala == true && municion != 0) {
            ...
            disparaEnemigo();
            setTimeout(function() {tiempoBala = true;}, 110);
        }
    }
}
